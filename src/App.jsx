...
export default function App() {
  ...
  const [partners, setPartners]   = useState([
    {name:"Carlos",pct:40},{name:"Gleidson",pct:40},{name:"Dhionata",pct:20}
  ]);
  ...
  function confirmBooking() {
    ...
    const sp = getSponsorInfo(me?.email||"");
    const spDayOk = sp ? isSponsorDayAllowed(sp,date) : false;
    const spUsed  = sp ? getSponsorUsedHours(me?.email||"") : 0;
    const spFree  = sp && spDayOk && spUsed < (sp.hoursPerWeek||1);
    const amount  = spFree ? 0 : (qualifies ? prices.regular : prices.common);
    setBookings(b => ({ ...b, [key]: {
      ...
      sponsorBooking:spFree, sponsorName:sp?sp.name:"",
      byAdmin:false,
      paid:spFree, paidLater:false, amount,
    }}));
    if (spFree) toast_("Reserva patrocinada! Gratuita ("+spUsed+"/"+(sp.hoursPerWeek||1)+"h usadas esta semana)");
    else if (sp && spDayOk && !spFree) toast_("Limite semanal atingido — cobrado R$"+prices.regular+"/h","info");
    setBookStep(4);
  }
  function resetFlow() {
  ...
  function getSponsorUsedHours(email) {
    ...
    return Object.values(bookings).filter(b=>b.email===email&&b.sponsorBooking&&b.date>=fmt(monday)).length;
  }
  function isSponsorDayAllowed(sp,date) {
    const dow=new Date(date).getDay(); const isWE=dow===0||dow===6;
    if(sp.dayType==="weekend") return isWE;
    if(sp.dayType==="weekday") return !isWE;
    return true;
  }
  function getDateRange(period) {
    const now=new Date(); const t=fmt(now); const yr=now.getFullYear();
    switch(period) {
      case "30d": {const s=new Date(now);s.setDate(s.getDate()-30);return {start:fmt(s),end:t};}
      case "month": return {start:yr+"-"+String(now.getMonth()+1).padStart(2,"0")+"-01",end:t};
      case "year":  return {start:yr+"-01-01",end:t};
      case "lastyear":{const y=yr-1;return {start:y+"-01-01",end:y+"-12-31"};}
  ...
  function copyDaySchedule(date) {
    try {
      var d=new Date(date); var dow=d.getDay(); var isWE=dow===0||dow===6;
      var hours=ALL_HOURS.filter(function(h){return isWE?(h>="06:00"&&h<="22:00"):(h>="17:00"&&h<="21:00");});
      var dl=DAYS_FULL[dow].toUpperCase()+" "+d.getDate()+"/"+String(d.getMonth()+1).padStart(2,"0");
      var lines2=["AGENDA "+dl+""]; 
      COURTS.forEach(function(ct){
        lines2.push("- "+ct.name);
        hours.forEach(function(h){
          var b=bookings[ct.id+"-"+date+"-"+h];
          var nm=b?(b.name.split(" ")[0]+" "+(b.sport==="Beach Tennis"?"BT":b.sport==="Futvolei"?"FV":"VB")):"";
          lines2.push(h+" - "+nm);
        });
      });
      navigator.clipboard.writeText(lines2.join("\n")).then(function(){toast_("Agenda do dia copiada!");});
    } catch(e){toast_("Erro ao copiar","error");}
  }

  function copyWeekSchedule(wkOff) {
    try {
      var wd=getWeekDates(wkOff);
      var d0=wd[0],d6=wd[6];
      var from=d0.getDate()+"/"+String(d0.getMonth()+1).padStart(2,"0");
      var to=d6.getDate()+"/"+String(d6.getMonth()+1).padStart(2,"0");
      var lines2=["AGENDA DE "+from+" A "+to];
      wd.forEach(function(d){
        var date=fmt(d); var dow=d.getDay(); var isWE=dow===0||dow===6;
        var hours=ALL_HOURS.filter(function(h){return isWE?(h>="06:00"&&h<="22:00"):(h>="17:00"&&h<="21:00");});
        lines2.push("");
      ...
      navigator.clipboard.writeText(lines2.join("\n")).then(function(){toast_("Agenda da semana copiada!");});
    } catch(e){toast_("Erro ao copiar","error");}
  }
        ...
        {view==="availability"&&(
          <div className="fi">
            ...
            <div style={{display:"flex",gap:10,marginTop:14,flexWrap:"wrap"}}>
              <button style={{...S.btnG,fontSize:13}} onClick={()=>copyDaySchedule(selDate)}>📋 Copiar dia (WhatsApp)</button>
              <button style={{...S.btnG,fontSize:13}} onClick={()=>copyWeekSchedule(weekOffset)}>📅 Copiar semana (WhatsApp)</button>
            </div>
          </div>
        )}
              ...
              <div>
                ...
                <div style={S.adminTabs}>
                  {[
                    ...
                    {id:"manual",     l:"✍️ Reserva Manual"},
                    {id:"valores",    l:"💰 Valores"},
                    {id:"patroc",     l:"🤝 Patrocinadores"},
                    {id:"despesas",   l:"💸 Despesas"},
                    {id:"gestao",     l:"📊 Gestão"},
                    {id:"heatmap",    l:"🔥 Horários"},
                ...
                {adminTab==="valores"&&(
                  <div style={{maxWidth:560}}>
                    <Section title="💰 Valores por hora de aluguel">
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
                        {[
                          ...
                          <div key={key} style={{background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,padding:20}}>
                            ...
                            <div style={{fontWeight:700,color,fontSize:15,marginBottom:4}}>{label}</div>
                      ...
                      <div style={S.infoBox}>
                        💡 Patrocinadores que ultrapassam o limite semanal são cobrados pelo valor <strong>Regular (R$ {prices.regular}/h)</strong>.
                      </div>
                      <button style={{...S.btnO,marginTop:16}} onClick={savePrices}>Salvar Preços</button>
                    </Section>
                    <Section title="👥 Divisão Societária">
                      <div style={{...S.infoBox,marginBottom:14}}>
                        Percentuais sobre o lucro. Total: <strong style={{color:partners.reduce((s,p)=>s+Number(p.pct),0)===100?"#22c55e":"#ef4444"}}>{partners.reduce((s,p)=>s+Number(p.pct),0)}%</strong>
                      </div>
                      {partners.map((p,i)=>(
                      ...
                      <div style={{display:"flex",gap:10,marginTop:8}}>
                        <button style={{...S.btnG,fontSize:13}} onClick={()=>setPartners(ps=>[...ps,{name:"",pct:0}])}>+ Sócio</button>
                        <button style={{...S.btnO,fontSize:13}} onClick={()=>toast_("Divisão salva!")}>Salvar</button>
                      </div>
                    </Section>
                  </div>
                )}
                  ...
                  <div style={{maxWidth:640}}>
                    <Section title="🤝 Patrocinadores">
                      <div style={{...S.infoBox,marginBottom:18}}>
                        Patrocinadores têm horas gratuitas/semana (ciclo seg-dom). Excedente cobra R$ <strong style={{color:"#f97316"}}>{prices.regular}</strong>/h. No relatório aparece como <strong style={{color:"#a855f7"}}>Patrocinador</strong>.
                      </div>
                      {sponsors.length===0&&<div style={{...S.infoBox,color:"#555",marginBottom:14}}>Nenhum patrocinador cadastrado.</div>}
                      {sponsors.map((sp,i)=>(
                        <div key={i} style={{background:"#141414",border:"1px solid #a855f7",borderRadius:14,padding:18,marginBottom:14}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                            <span style={{color:"#a855f7",fontWeight:700}}>🤝 Patrocinador {i+1}</span>
                            <div style={{display:"flex",gap:8}}>
                              <button style={{background:sp.active?"#14532d":"#2a1010",color:sp.active?"#22c55e":"#ef4444",border:"none",borderRadius:8,padding:"5px 12px",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}
                                onClick={()=>setSponsors(ss=>ss.map((x,j)=>j===i?{...x,active:!x.active}:x))}>
                                {sp.active?"✓ Ativo":"✗ Inativo"}</button>
                              <button style={{...S.aBlue,padding:"6px 10px"}} onClick={()=>setSponsors(ss=>ss.filter((_,j)=>j!==i))}>✕</button>
                            </div>
                          </div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                            <F label="Nome/Empresa" v={sp.name||""} set={e=>setSponsors(ss=>ss.map((x,j)=>j===i?{...x,name:e.target.value}:x))} ph="Nome"/>
                            <F label="Email da conta" type="email" v={sp.email||""} set={e=>setSponsors(ss=>ss.map((x,j)=>j===i?{...x,email:e.target.value}:x))} ph="email@patroc.com"/>
                          </div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
                            <div style={S.fg}>
                              <label style={S.lbl}>Horas grátis/semana</label>
                              <input type="number" min="1" max="20" style={{...S.inp,textAlign:"center",fontWeight:700,fontSize:18}}
                            ...
                            <div style={S.fg}>
                              <label style={S.lbl}>Usado esta semana</label>
                              <div style={{...S.inp,textAlign:"center",fontWeight:700,fontSize:16,cursor:"default",
                          ...
                          {sp.email&&(
                            <div style={{...S.infoBox,marginTop:10,fontSize:12}}>
                              {getSponsorUsedHours(sp.email)>=(sp.hoursPerWeek||1)
                                ?<span style={{color:"#ef4444"}}>⚠️ Limite atingido — próximas reservas cobram R${prices.regular}/h</span>
                                :<span style={{color:"#22c55e"}}>✅ {(sp.hoursPerWeek||1)-getSponsorUsedHours(sp.email)}h gratuita(s) disponível(is)</span>}
                            </div>
                          )}
                        </div>
                      ))}
                      <button style={S.btnO} onClick={()=>setSponsors(ss=>[...ss,{name:"",email:"",hoursPerWeek:2,dayType:"any",active:true}])}>
                ...
                {adminTab==="despesas"&&(
                  <div style={{maxWidth:680}}>
                    ...
                    <Section title="📋 Histórico de Despesas">
                      {expenses.length===0&&<div style={S.infoBox}>Nenhuma despesa registrada.</div>}
                      <div style={{maxHeight:380,overflowY:"auto"}}>
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                          {expenses.length>0&&<thead><tr style={{background:"#1a1a1a"}}>
                            {["Data","Hora","Material","Motivo","Quem Pagou","Valor",""].map(h=>(
                              <th key={h} style={{padding:"7px 10px",textAlign:"left",color:"#888",fontWeight:600,borderBottom:"1px solid #2a2a2a"}}>{h}</th>
                            ))}
                          </tr></thead>}
                          <tbody>
                            {expenses.map((e,i)=>(
                              <tr key={i} style={{borderBottom:"1px solid #141414"}}>
                                <td style={{padding:"6px 10px",color:"#ccc"}}>{e.date}</td>
                                <td style={{padding:"6px 10px",color:"#ccc"}}>{e.hour}</td>
                                <td style={{padding:"6px 10px",color:"#fff",fontWeight:600}}>{e.material}</td>
                                <td style={{padding:"6px 10px",color:"#888"}}>{e.motivo}</td>
                                <td style={{padding:"6px 10px",color:"#ccc"}}>{e.quemPagou}</td>
                                <td style={{padding:"6px 10px",color:"#ef4444",fontWeight:700}}>R$ {e.valor}</td>
                                <td style={{padding:"6px 10px"}}><button style={{...S.aBlue,fontSize:10}} onClick={()=>setExpenses(es=>es.filter((_,j)=>j!==i))}>✕</button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      </div>
                      {expenses.length>0&&<div style={{marginTop:10,color:"#ef4444",fontWeight:700}}>Total: R$ {expenses.reduce((s,e)=>s+Number(e.valor||0),0).toLocaleString("pt-BR")}</div>}
                    </Section>
                  </div>
                )}
    ...
    <div style={S.fg}>
      ...
      <div style={{position:"relative"}}>
        ...
        <button type="button" onClick={()=>setShow(s=>!s)}
          style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#888",fontSize:18,lineHeight:1}}>
          {show?"🙈":"👁️"}
        </button>
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
        ...
        <F label="Hora" v={form.hour} set={e=>setForm(f=>({...f,hour:e.target.value}))} ph="14:00"/>
        <F label="Valor R$ *" type="number" v={form.valor} set={e=>setForm(f=>({...f,valor:e.target.value}))} ph="0"/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
        <F label="Material *" v={form.material} set={e=>setForm(f=>({...f,material:e.target.value}))} ph="ex: Rede de vôlei"/>
        <F label="Motivo" v={form.motivo} set={e=>setForm(f=>({...f,motivo:e.target.value}))} ph="ex: Manutenção"/>
      </div>
      <F label="Quem pagou" v={form.quemPagou} set={e=>setForm(f=>({...f,quemPagou:e.target.value}))} ph="Nome"/>
...
function PaymentEditor({bookings,setEditPayModal}) {
  const [filter,setFilter]=useState("all");
  const [pS,setPS]=useState(""); const [pE,setPE]=useState("");
  const all=Object.entries(bookings).sort((a,b)=>a[1].date.localeCompare(b[1].date));
  const filt=all.filter(([,b])=>{
    if(filter==="pending"&&(b.paid||b.paidLater)) return false;
    if(pS&&b.date<pS) return false;
    if(pE&&b.date>pE) return false;
    return true;
  });
  const COURTS2=[{id:"sicoob",name:"Sicoob"},{id:"tropical",name:"Tropical Net"}];
  return (
    <div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}>
        <button onClick={()=>setFilter("all")} style={{background:filter==="all"?"#f97316":"#1a1a1a",color:filter==="all"?"#fff":"#888",border:"1px solid "+(filter==="all"?"#f97316":"#2a2a2a"),borderRadius:8,padding:"6px 14px",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>Mostrar Tudo</button>
        <button onClick={()=>setFilter("pending")} style={{background:filter==="pending"?"#ef4444":"#1a1a1a",color:filter==="pending"?"#fff":"#888",border:"1px solid "+(filter==="pending"?"#ef4444":"#2a2a2a"),borderRadius:8,padding:"6px 14px",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>⚠️ Só Pendentes</button>
        <input type="date" style={{...S.inp,width:"auto",fontSize:12}} value={pS} onChange={e=>setPS(e.target.value)}/>
        <span style={{color:"#666"}}>até</span>
        <input type="date" style={{...S.inp,width:"auto",fontSize:12}} value={pE} onChange={e=>setPE(e.target.value)}/>
        <span style={{color:"#666",fontSize:12}}>{filt.length} registros</span>
      </div>
      <div style={{maxHeight:400,overflowY:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#1a1a1a"}}>
            {["Data","Hora","Nome","Quadra","Esporte","Valor","Status",""].map(h=>(
          ...
          <tbody>
            {filt.map(([key,b])=>{
              const ct=COURTS2.find(x=>x.id===b.courtId);
              const st=b.paid?"✅ Pago":b.paidLater?"⏳ Depois":b.payError?"❌ Erro":"⚠️ Pend.";
              const sc=b.paid?"#22c55e":b.paidLater?"#60a5fa":b.payError?"#ef4444":"#f59e0b";
              return (
                <tr key={key} style={{borderBottom:"1px solid #141414"}}>
                  <td style={{padding:"6px 8px",color:"#ccc"}}>{b.date}</td>
                  <td style={{padding:"6px 8px",color:"#ccc"}}>{b.hour}</td>
                  <td style={{padding:"6px 8px",color:"#fff",fontWeight:600}}>{b.name}</td>
                  <td style={{padding:"6px 8px",color:"#aaa"}}>{ct?.name}</td>
                  <td style={{padding:"6px 8px",color:"#aaa"}}>{b.sport}</td>
                  <td style={{padding:"6px 8px",color:"#f97316",fontWeight:700}}>R${b.amount}</td>
                  <td style={{padding:"6px 8px",color:sc,fontWeight:700}}>{st}</td>
        ...
        {filt.length===0&&<div style={{padding:20,color:"#555",textAlign:"center"}}>Nenhuma reserva encontrada.</div>}
      </div>
    </div>
  );
}
        ...
        {wd.map((d,i)=>{
          ...
          return (
            <button key={i} disabled={past} onClick={()=>setSel(ds)}
              ...
              <div style={{fontWeight:700,fontSize:12}}>{d.getDate()}</div>
